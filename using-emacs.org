...
* How to search for patterns in files in emacs using consult

Consult is the search package that you use.

How to look for some pattern into some precise list of files
You can use the prefix command with ~consult-grep~ and ~consult-ripgrep~. Then you can list the different files you want to look into, comma-separated, and here it is.

You can also use directly filtering patterns, as those commands support the command line parameters their utility uses.
For example you can search in all files ending with =.org= by typing in the consult-ripgrep prompt :
...
As the previous point said, a macro ring exists. You are able to jump between one macro you've recorded or another without having to save them before and bind them.
To do that :
...
Start by listing them with : =list-keyboard-macros= and here it's you can modify them


* Building a new ts mode
...
While working with lsp-mode and lsp-java in emacs, you may face some cryptic jdtls java errors, please bro, when that happens try to update the version of jdtls which is downloaded through the =lsp-java-jdt-download-url= (or similar). You'll just have to provide the proper link and test. You should do that before any configuration rabbit hole.

A full example of configuration that I've used for a project :
...
#+begin_src emacs-lisp
  (with-eval-after-load 'lsp-java
    (setq lsp-java-configuration-runtimes '[(:name "JavaSE-21"
                                                   ...
                                                   :default t)])
    (require 'lsp-java-boot) ;; was a springboot project

    ;; I've tried multiple version until one work
    ;; the one that was considering both the packages of the current project and the one from the default java libraries
    (setq lsp-java-jdt-download-url "https://www.eclipse.org/downloads/download.php?file=jdtls/milestones/1.32.0/jdt-language-server-1.32.0-202402011424.tar.gz")

    (add-hook 'lsp-mode-hook #'lsp-lens-mode)
    (add-hook 'java-ts-mode-hook #'lsp-java-boot-lens-mode)

    (let ((lombok-jar
           (expand-file-name "/Users/test/.m2/repository/org/projectlombok/lombok/1.18.36/lombok-1.18.36.jar")))
      (unless (file-exists-p lombok-jar)
        (error "Lombok-Jar not found: %s" lombok-jar))

      (setq lsp-java-jdt-ls-lombok-support-enabled t)
...
To begin with I thought company was slow, but that's not the case, it's just the default that are not optimized you could check my configuration to know what to do.

Second point, if you moved from corfu you know that you can corfu-insert-separator to fuzzy easily in the completion candidates. I didn't knew that company has it too through the binding : =C-M-s= by default. 
